...
risk_factors_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    climate_data <- reactive({
      ...
      data.frame(
        year = years,
        cases = base_cases * (1 + climate_trend * 0.5) + rnorm(length(years), 0, 2000),
        ...
        dplyr::mutate(
          climate_value = switch(input$climate_factor,
            "temp" = temp,
            "precip" = precip,
            "humidity" = humidity,
            "season" = season,
    ...
    social_data <- reactive({
      factors <- c("Population Density", "Outdoor Recreation", 
                  "Forest Coverage", "Urban Sprawl", "Pet Ownership")
    ...
    output$climate_correlation <- echarts4r::renderEcharts4r({
      ...
      factor_name <- switch(input$climate_factor,
      ...
      data |>
        ...
        echarts4r::e_title(
          ...
          subtext = sprintf("Years %d-%d", input$year_range[1], input$year_range[2])
        ) |>
        echarts4r::e_x_axis(
          name = factor_name,
          nameLocation = "middle",
